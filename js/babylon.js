var babylon = new function() {
  ...
  this.createScene = function () {
    ...
    // Disable keyboard inputs - arrow keys should only control the robot, not the camera
    cameraArc.inputs.removeByType("ArcRotateCameraKeyboardMoveInput");
    ...
    // Add lights to the scene (restore from localStorage if saved)
    ...
    var dirIntensity = savedDir !== null ? savedDir / 100 : 0.8;
    var hemiIntensity = savedHemi !== null ? savedHemi / 100 : 0.5;
    var shadowDarkness = savedShadow !== null ? savedShadow / 100 : 0.5;

    self.lightHemi = new BABYLON.HemisphericLight('light1', new BABYLON.Vector3(1, 1, 0), scene);
    self.lightHemi.groundColor = new BABYLON.Color3(0.1, 0.2, 0.1);
    self.lightHemi.intensity = hemiIntensity;

    self.lightDir = new BABYLON.DirectionalLight('DirectionalLight', new BABYLON.Vector3(-1, -1, -1), scene);
    self.lightDir.position.y = 100;
    self.lightDir.position.x = 400;
    self.lightDir.position.z = 400;
    self.lightDir.intensity = dirIntensity;
    self.lightDir.autoCalcShadowZBounds = true;

    // Sky color responds to Sun intensity
    self.updateSkyColor = function(sunIntensity) {
      var t = Math.min(sunIntensity / 0.8, 1.0); // normalize to default max
      scene.clearColor = new BABYLON.Color4(
        0.05 + 0.35 * t,  // R: 0.05 (dark) to 0.40 (sky blue)
        0.05 + 0.55 * t,  // G: 0.05 (dark) to 0.60
        0.15 + 0.75 * t,  // B: 0.15 (dark blue) to 0.90
        1.0
      );
    };
    self.updateSkyColor(dirIntensity);

    // Shadows
    scene.shadowGenerator = new BABYLON.ShadowGenerator(512, self.lightDir);
    scene.shadowGenerator.forceBackFacesOnly = true;
    scene.shadowGenerator.darkness = shadowDarkness;
  ...
  this.setCameraMode = function(mode) {
    ...
    if (self.cameraMode == 'follow') {
      ...
      // Keyboard control removed - arrow keys now control robot via simPanel
      self.cameraArc.angularSensibilityX = self.cameraArc.angularSensibility;
      self.cameraArc.angularSensibilityY = self.cameraArc.angularSensibility;
      ...
      self.cameraArc.angularSensibilityX = self.cameraArc.angularSensibility;
      self.cameraArc.angularSensibilityY = self.cameraArc.angularSensibility;
    } else if (self.cameraMode == 'chase') {
      // Chase camera: behind robot, rotates with robot heading, low angle, zoomed in
      self.cameraArc.origAlpha = null;
      self.cameraArc.origBeta = null;
      self.cameraArc.lockedTarget = robot.body;
      self.cameraArc.alpha = -Math.PI / 2;  // Behind robot (will be updated in render)
      self.cameraArc.beta = Math.PI / 3.2;  // Slightly higher angle (~56 degrees)
      self.cameraArc.radius = 70;           // Zoomed in
      self.cameraArc._panningMouseButton = 1;
      babylon.cameraArc.mode = BABYLON.Camera.PERSPECTIVE_CAMERA;
      self.cameraArc.angularSensibilityX = self.cameraArc.angularSensibility;
      self.cameraArc.angularSensibilityY = self.cameraArc.angularSensibility;
    } else if (self.cameraMode == 'behind') {
      // Behind camera: low angle, zoomed in, but doesn't rotate with robot
      if (self.cameraArc.origAlpha !== null) {
        self.cameraArc.alpha = self.cameraArc.origAlpha;
      }
      if (self.cameraArc.origBeta !== null) {
      ...
      self.cameraArc.origBeta = null;
      self.cameraArc.lockedTarget = robot.body;
      self.cameraArc.beta = Math.PI / 3.2;  // Low angle (~56 degrees)
      self.cameraArc.radius = 70;           // Zoomed in
      self.cameraArc._panningMouseButton = 1;
      babylon.cameraArc.mode = BABYLON.Camera.PERSPECTIVE_CAMERA;
      self.cameraArc.angularSensibilityX = self.cameraArc.angularSensibility;
  ...
  this.resetScene = function() {
    // If engine/scene not initialized yet, wait and try again
    if (!self.engine || !self.scene) {
      return new Promise(function(resolve) {
        setTimeout(function() {
          self.resetScene().then(resolve);
        }, 100);
      });
    }

    // Save camera position and rotations (if camera exists)
    let pos, target, rot, up, mode;
    if (self.cameraArc) {
      pos = self.cameraArc.position;
      target = self.cameraArc.target;
      rot = self.cameraArc.absoluteRotation;
      up = self.cameraArc.upVector;
      mode = self.cameraMode;
    }

    // Notify sensor visualization before scene dispose
    if (typeof sensorViz !== 'undefined') {
      sensorViz.onSceneReset();
    }
    ...
    return self.loadMeshes(self.scene).then(function(){
      // Restore camera (if we saved its state earlier)
      if (pos) {
  ...
  this.render = function() {
    ...
    if (self.cameraMode == 'chase' && typeof robot !== 'undefined' && robot.body) {
      // Get robot's Y rotation from quaternion
      var rotationQuaternion = robot.body.rotationQuaternion;
      if (rotationQuaternion) {
        // Convert quaternion to euler Y angle
        var euler = rotationQuaternion.toEulerAngles();
        // Target alpha to be behind robot (offset by PI)
        var targetAlpha = -euler.y - Math.PI / 2;

        // Smooth lerping for camera rotation
        // Handle angle wrapping (avoid spinning the long way around)
        var currentAlpha = self.cameraArc.alpha;
        var diff = targetAlpha - currentAlpha;

        // Normalize the difference to [-PI, PI]
        while (diff > Math.PI) diff -= 2 * Math.PI;
        while (diff < -Math.PI) diff += 2 * Math.PI;

        // Lerp factor (0.05 = slow/smooth, 0.2 = faster/responsive)
        var lerpFactor = 0.08;
        self.cameraArc.alpha = currentAlpha + diff * lerpFactor;
      }
    }
