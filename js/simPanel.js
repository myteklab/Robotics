var simPanel = new function() {
  ...
  this.init = function() {
    ...
    self.$fps = $('.fps');
    self.$lightingPanel = $('.lightingPanel');
    self.$lightingIcon = $('.lightingPanel > .icon');
    self.$speedIndicator = $('.speedIndicator');
    self.$speedBar = $('.speedBar');
    self.$speedValue = $('.speedValue');
    self.$gripIndicator = $('.gripIndicator');

    setOnClickAnimation([self.$runSim, self.$world, self.$reset, self.$camera, self.$ruler, self.$sensors, self.$joystickIcon, self.$hubButtonsIcon, self.$lightingIcon]);

    // Speed indicator update interval
    self.speedUpdateTimer = setInterval(self.updateSpeedIndicator, 100);
    ...
    if (self.$lightingPanel.length > 0) {
      self.$lightingIcon.click(self.toggleLighting);
      self.setupLightingControls();
    }

    // Setup keyboard capture for custom key detection blocks
    self.setupKeyboardCapture();
  ...
  this.toggleLighting = function() {
    self.$lightingPanel.toggleClass('closed');
  };

  // setup lighting controls
  this.setupLightingControls = function() {
    var $dirSlider = self.$lightingPanel.find('.lightDirIntensity');
    var $hemiSlider = self.$lightingPanel.find('.lightHemiIntensity');
    var $shadowSlider = self.$lightingPanel.find('.shadowDarkness');

    var savedDir = localStorage.getItem('gearsbot_lightDir');
    var savedHemi = localStorage.getItem('gearsbot_lightHemi');
    var savedShadow = localStorage.getItem('gearsbot_shadowDarkness');

    if (savedDir !== null) {
      $dirSlider.val(savedDir);
    }
    if (savedHemi !== null) {
      $hemiSlider.val(savedHemi);
    }
    if (savedShadow !== null) {
      $shadowSlider.val(savedShadow);
    }

    $dirSlider.on('input', function() {
      var intensity = this.value / 100;
      if (babylon.lightDir) {
        babylon.lightDir.intensity = intensity;
      }
      if (babylon.updateSkyColor) {
        babylon.updateSkyColor(intensity);
      }
      localStorage.setItem('gearsbot_lightDir', this.value);
    });

    $hemiSlider.on('input', function() {
      if (babylon.lightHemi) {
        babylon.lightHemi.intensity = this.value / 100;
      }
      localStorage.setItem('gearsbot_lightHemi', this.value);
    });

    $shadowSlider.on('input', function() {
      if (babylon.scene && babylon.scene.shadowGenerator) {
        babylon.scene.shadowGenerator.darkness = this.value / 100;
      }
      localStorage.setItem('gearsbot_shadowDarkness', this.value);
    });
  };
  ...
  this.initSensorsPanel = function() {
    ...
    while (sensor = robot.getComponentByPort('in' + i)) {
      ...
      } else if (sensor.type == 'Display') {
        let $div = $(
          '<div class="sensorReading displayReading">' +
            '<div class="sensorType">' + sensor.port + ': Display</div>' +
            '<div class="displayOverlay">' +
              '<div class="displayLine" data-line="0"></div>' +
              '<div class="displayLine" data-line="1"></div>' +
              '<div class="displayLine" data-line="2"></div>' +
              '<div class="displayLine" data-line="3"></div>' +
              '<div class="displayLine" data-line="4"></div>' +
            '</div>' +
          '</div>'
        );
        tmp = [$div, $div.find('.displayLine')];
      } else {
        console.log(sensor);
      }
  ...
  this.updateSensorsPanel = function() {
    ...
    self.sensors.forEach(function(sensor) {
      ...
      } else if (sensor[0].type == 'ClawActuator') {
        sensor[1][0].text(Math.round(sensor[0].position));
      } else if (sensor[0].type == 'LED') {
        sensor[1][0].text(sensor[0].isOn ? 'On' : 'Off');
        sensor[1][1].text(sensor[0].color[0] + ', ' + sensor[0].color[1] + ', ' + sensor[0].color[2]);
      } else if (sensor[0].type == 'CameraSensor') {
        sensor[1][0].text(sensor[0].active ? 'Active' : 'Inactive');
      } else if (sensor[0].type == 'Display') {
        var lines = sensor[0].getLines();
        for (var i = 0; i < 5; i++) {
          var lineText = lines[i] || '\u00A0';
          sensor[1].eq(i).text(lineText);
        }
      }
    });
  };
  ...
  this.switchCamera = function(e) {
    ...
    } else if (e.currentTarget.classList.contains('cameraBehind')) {
      babylon.setCameraMode('behind');
      self.$camera.html('<svg class="icon-cameraBehind" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="currentColor" style="display:block;margin:auto;"><path d="M2 7h12v9H2z" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="3" y="8.5" width="2" height="2.5" rx="0.3"/><rect x="6" y="8.5" width="2" height="2.5" rx="0.3"/><rect x="9" y="8.5" width="2" height="2.5" rx="0.3"/><rect x="3" y="11.5" width="2" height="2.5" rx="0.3"/><rect x="6" y="11.5" width="2" height="2.5" rx="0.3"/><rect x="9" y="11.5" width="2" height="2.5" rx="0.3"/><path d="M14 9.5l3-1.5v7l-3-1.5z" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="18.5" y="8" width="4" height="5" rx="0.8" fill="currentColor"/><rect x="18" y="13.5" width="2" height="1.5" rx="0.3" fill="currentColor"/><rect x="21" y="13.5" width="2" height="1.5" rx="0.3" fill="currentColor"/></svg>');

    } else if (e.currentTarget.classList.contains('cameraChase')) {
      babylon.setCameraMode('chase');
      self.$camera.html('<svg class="icon-cameraChase" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="currentColor" style="display:block;margin:auto;"><path d="M4 6h12v9H4z" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="5" y="7.5" width="2" height="3" rx="0.3"/><rect x="8" y="7.5" width="2" height="3" rx="0.3"/><rect x="11" y="7.5" width="2" height="3" rx="0.3"/><rect x="5" y="11" width="2" height="3" rx="0.3"/><rect x="8" y="11" width="2" height="3" rx="0.3"/><rect x="11" y="11" width="2" height="3" rx="0.3"/><path d="M16 8.5l4-2v8l-4-2z" fill="none" stroke="currentColor" stroke-width="1.5"/><line x1="1" y1="8" x2="3" y2="8" stroke="currentColor" stroke-width="1.2"/><line x1="0" y1="10.5" x2="3" y2="10.5" stroke="currentColor" stroke-width="1.2"/><line x1="1" y1="13" x2="3" y2="13" stroke="currentColor" stroke-width="1.2"/><rect x="21" y="9" width="2.5" height="3" rx="0.5" fill="currentColor"/></svg>');
  ...
  this.toggleCameraSelector = function() {
    let child = self.$camera.children()[0];
    let classStr = '';
    if (child.tagName.toLowerCase() === 'svg') {
      classStr = child.getAttribute('class') || '';
    } else {
      classStr = child.className || '';
    }
    let current = classStr.replace('icon-', '');
  ...
  this.loadWorld = function(json) {
    try {
      ...
      console.log('World load failed:', e);
    }
  };
  ...
  // Get current world config for platform save
  this.getWorldConfig = function() {
    if (!babylon || !babylon.world) return null;
    return {
      worldName: babylon.world.name,
      options: babylon.world.options || {}
    };
  };

  // Set world config from platform load
  this.setWorldConfig = function(config) {
    if (!config || !config.worldName) return;
    self.loadWorld(JSON.stringify(config));
  };
  ...
  this.resetSim = function(resetPython) {
    ...
    // Clear keyboard keys on reset
    if (typeof robot !== 'undefined' && robot.clearKeyboardKeys) {
      robot.clearKeyboardKeys();
    }
  ...
  // Setup keyboard capture for custom key detection in code blocks
  this.setupKeyboardCapture = function() {
    const RESERVED_KEYS = [
      'F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12',
      'Tab', 'Escape', 'CapsLock', 'NumLock', 'ScrollLock', 'Pause',
      'Insert', 'Delete', 'Home', 'End', 'PageUp', 'PageDown',
      'Control', 'Alt', 'Meta', 'Shift', 'ContextMenu'
    ];

    const simPanelEl = document.getElementById('simPanel');
    const canvas = document.getElementById('renderCanvas');
    if (!simPanelEl || !canvas) return;

    if (typeof babylon !== 'undefined' && babylon.cameraArc && babylon.cameraArc.inputs) {
      var keyboardInput = babylon.cameraArc.inputs.attached.keyboard;
      if (keyboardInput) {
        babylon.cameraArc.inputs.remove(keyboardInput);
      }
    }
    ...
    let targetSpeed = { left: 0, right: 0 };
    let currentSpeed = { left: 0, right: 0 };
    let easingInterval = null;
    const EASING_RATE = 0.15;
    const EASING_INTERVAL_MS = 20;

    function moveTank(leftWheel, rightWheel) {
      if (typeof robot === 'undefined' || !robot.leftWheel || !robot.rightWheel) return;
    ...
    function startEasing() {
      if (easingInterval) return;

      easingInterval = setInterval(function() {
        currentSpeed.left += (targetSpeed.left - currentSpeed.left) * EASING_RATE;
        currentSpeed.right += (targetSpeed.right - currentSpeed.right) * EASING_RATE;

        if (Math.abs(currentSpeed.left) < 5) currentSpeed.left = 0;
        if (Math.abs(currentSpeed.right) < 5) currentSpeed.right = 0;

        moveTank(Math.round(currentSpeed.left), Math.round(currentSpeed.right));

        if (targetSpeed.left === 0 && targetSpeed.right === 0 &&
            currentSpeed.left === 0 && currentSpeed.right === 0) {
    ...
    function driveRobot() {
      let l = 0, r = 0;
      const SPEED = shiftHeld ? 200 : 800;

      if (arrowKeys.up) {
        l = SPEED;
        r = SPEED;
        if (arrowKeys.left) {
          l = shiftHeld ? SPEED * 0.3 : 0;
        } else if (arrowKeys.right) {
          r = shiftHeld ? SPEED * 0.3 : 0;
        }
    ...
    function isSimulatorTabActive() {
      return simPanelEl.classList.contains('active') ||
             simPanelEl.offsetParent !== null;
    }
    ...
    function isEditorFocused() {
      ...
      if (activeEl.classList.contains('ace_text-input')) {
        return true;
      }
      if (activeEl.closest('.blocklyEditor')) {
        return true;
      }
      return false;
    }
    ...
    window.addEventListener('keydown', function(e) {
      if (!isSimulatorTabActive()) return;

      if (RESERVED_KEYS.includes(e.key) && e.key !== 'Shift') return;
      ...
      if (e.key === 'ArrowLeft') {
        if (!e.repeat) autoStartSimIfNeeded();
        arrowKeys.left = true;
        driveRobot();
        e.preventDefault();
        return false;
      ...
      if (typeof robot !== 'undefined' && robot.setKeyPressed) {
        robot.setKeyPressed(e.key, true);
      }
    }, true);
    ...
    window.addEventListener('keyup', function(e) {
      ...
      if (typeof robot !== 'undefined' && robot.setKeyPressed) {
        robot.setKeyPressed(e.key, false);
      }
    }, true);
    ...
    window.addEventListener('blur', function() {
      ...
      shiftHeld = false;
      targetSpeed = { left: 0, right: 0 };
      currentSpeed = { left: 0, right: 0 };
      if (easingInterval) {
        clearInterval(easingInterval);
        easingInterval = null;
      }
    ...
    document.querySelectorAll('#navBlocks, #navPython').forEach(function(tab) {
  ...
  this.toggleConsole = function() {
    ...
    var isOpen = self.$console.hasClass('open');
    self.$speedIndicator.toggleClass('console-open', isOpen);
    self.$gripIndicator.toggleClass('console-open', isOpen);
  };
  ...
  // Update speed indicator based on robot wheel speeds
  this.updateSpeedIndicator = function() {
    if (typeof robot === 'undefined' || !robot.leftWheel || !robot.rightWheel) {
      return;
    }

    let leftSpeed = Math.abs(robot.leftWheel.speed_sp || 0);
    let rightSpeed = Math.abs(robot.rightWheel.speed_sp || 0);
    let avgSpeed = (leftSpeed + rightSpeed) / 2;

    let percentage = Math.min(100, Math.round(avgSpeed / 800 * 100));

    self.$speedBar.css('width', percentage + '%');
    self.$speedValue.text(percentage + '%');

    if (percentage < 30) {
      self.$speedBar.css('background', 'linear-gradient(90deg, #4a9eff, #6eb5ff)');
    } else if (percentage < 70) {
      self.$speedBar.css('background', 'linear-gradient(90deg, #4a9eff, #9b59b6)');
    } else {
      self.$speedBar.css('background', 'linear-gradient(90deg, #9b59b6, #e74c3c)');
    }
  };

  // Update grip indicator
  this.updateGripIndicator = function(isGripping) {
    if (self.$gripIndicator.length === 0) return;

    if (isGripping) {
      self.$gripIndicator.addClass('active').text('GRIP');
    } else {
      self.$gripIndicator.removeClass('active').text('');
    }
  };
}
