var World_Base = function() {
  ...
  this.defaultOptions = {
    ...
    arenaStartRot: null,
    // Metadata properties (used for save/load, not rendering)
    worldName: null,
    options: null
  };
  ...
  this.load = function (scene) {
    ...
    return new Promise(async function(resolve, reject) {
      ...
      // Show the info panel if score zones exist
      if (self.scoreZones.length > 0 && typeof self.panel != 'undefined') {
        self.panel.showWorldInfoPanel();
        self.drawScore(true);
      }
  ...
  this.addObject = async function(scene, object, indexObj) {
    ...
    if (options.type == 'compound') {
      ...
      mesh = self.addZone(scene, options, indexObj.index);
      indexObj.index++;
      return mesh;
    } else {
  ...
  // Add a zone object (no physics, semi-transparent)
  this.addZone = function(scene, options, index) {
    if (options.position.length < 3) {
      options.position.push(0);
    }
    ...
    // Parse color - supports 8-char hex with alpha (#RRGGBBAA)
    let colorHex = options.color || '#00ff0060';
    let r = 0, g = 1, b = 0, a = 0.35;
    if (colorHex.length >= 7) {
      r = parseInt(colorHex.substr(1, 2), 16) / 255;
      g = parseInt(colorHex.substr(3, 2), 16) / 255;
      b = parseInt(colorHex.substr(5, 2), 16) / 255;
    }
    if (colorHex.length >= 9) {
      a = parseInt(colorHex.substr(7, 2), 16) / 255;
    }

    let zoneMesh = BABYLON.MeshBuilder.CreateBox('worldBaseObject_zone' + index, {
      width: options.size[0],
      depth: options.size[1],
      height: options.size[2]
    }, scene);

    zoneMesh.position = new BABYLON.Vector3(
      options.position[0],
      options.position[2],
      options.position[1]
    );
    zoneMesh.rotation = new BABYLON.Vector3(rotationRad[0], rotationRad[1], rotationRad[2]);

    let material = new BABYLON.StandardMaterial('zoneMat_' + index, scene);
    material.diffuseColor = new BABYLON.Color3(r, g, b);
    material.specularColor = new BABYLON.Color3(0, 0, 0);
    material.emissiveColor = new BABYLON.Color3(r * 0.3, g * 0.3, b * 0.3);
    material.alpha = a;
    ...
    zoneMesh.zoneOptions = options;
    zoneMesh.zoneIndex = self.scoreZones.length;
    zoneMesh.zoneOriginalAlpha = a;

    // Optional label
    if (options.showLabel && options.label) {
      let labelText = options.label;
      let planeWidth = Math.max(labelText.length * 1.5, 5);
      let labelPlane = BABYLON.MeshBuilder.CreatePlane('zoneLabel_' + index, {
        width: planeWidth,
        height: 3
      }, scene);
      labelPlane.parent = zoneMesh;
      labelPlane.position.y = options.size[2] / 2 + 2;
      labelPlane.billboardMode = BABYLON.Mesh.BILLBOARDMODE_ALL;
      labelPlane.isPickable = false;

      let labelTexture = new BABYLON.DynamicTexture('zoneLabelTex_' + index, {
        width: planeWidth * 64,
        height: 192
      }, scene);
      labelTexture.hasAlpha = true;
      let ctx = labelTexture.getContext();
      ctx.clearRect(0, 0, planeWidth * 64, 192);
      labelTexture.drawText(labelText, null, 140, 'bold 100px Arial', 'white', 'transparent', true);

      let labelMat = new BABYLON.StandardMaterial('zoneLabelMat_' + index, scene);
  ...
  this.addModel = async function(scene, options) {
    ...
    if (options.modelURL) {
      let urlLower = options.modelURL.toLowerCase();
  ...
  self.render = function(delta){
    ...
    self.renderScoreZones();
  }

  // Render score zone detection
  self.renderScoreZones = function() {
    if (!self.scoreState.running || self.scoreZones.length === 0) return;

    self.scoreZones.forEach(function(zoneMesh, idx) {
      ...
      if (options.repeatable && zoneState.lastScoreTime > 0) {
        if (Date.now() - zoneState.lastScoreTime < 1000) return;
      }
      ...
      if (scored) {
        zoneState.triggered = true;
        zoneState.score += options.points;
        zoneState.lastScoreTime = Date.now();
        self.scoreState.totalScore += options.points;
  ...
  // Check if any robot body is inside the zone
  self.checkRobotInZone = function(zoneMesh) {
    if (typeof robots == 'undefined' || !robots || robots.length === 0) return false;

    for (let i = 0; i < robots.length; i++) {
      if (!robots[i] || !robots[i].body) continue;
      if (zoneMesh.intersectsPoint(robots[i].body.absolutePosition)) {
        return true;
      }
    }
    return false;
    ...
    for (let i = 0; i < meshes.length; i++) {
      ...
      if (typeof robots != 'undefined' && robots) {
        for (let r = 0; r < robots.length; r++) {
          if (!robots[r] || !robots[r].components) continue;
          for (let c = 0; c < robots[r].components.length; c++) {
            if (robots[r].components[c].grippedMesh === mesh) {
              isGripped = true;
              break;
            }
          }
  ...
  self.checkObjectDwell = function(zoneMesh, zoneState, options) {
    ...
    if (objectInZone) {
      if (zoneState.dwellStart === null) {
        zoneState.dwellStart = Date.now();
      }
      let elapsed = (Date.now() - zoneState.dwellStart) / 1000;
      if (elapsed >= (options.dwellTime || 0)) {
        zoneState.dwellStart = null;
        return true;
      }
    } else {
      zoneState.dwellStart = null;
  ...
  // Flash zone material when scored - uses scale pulse for reliable visibility
  self.triggerZoneFeedback = function(zoneMesh) {
    if (!zoneMesh) return;

    let originalScale = zoneMesh.scaling.clone();
    let pulseCount = 0;

    function pulse() {
      if (pulseCount >= 4) {
        zoneMesh.scaling = originalScale;
        return;
      }
      if (pulseCount % 2 === 0) {
        // Pulse larger
        zoneMesh.scaling = new BABYLON.Vector3(
  ...
  self.drawScore = function(rebuild) {
    if (typeof self.panel == 'undefined') return;

    // Check if $score is still attached to the DOM
    let needsRebuild = rebuild || typeof self.$score == 'undefined';
    if (!needsRebuild && self.$score && self.$score.closest('body').length === 0) {
      needsRebuild = true;
    }

    if (needsRebuild) {
      // Remove any existing score row to prevent duplicates
      $('.worldInfo .scoreRow').remove();

      let $info = $(
        '<div class="mono row scoreRow">' +
          '<span class="scoreLabel">SCORE</span>' +
          '<span class="scoreValue">0</span>' +
        '</div>'
      );
      self.panel.drawWorldInfo($info);
      self.$score = $info.find('.scoreValue');
    }

    let score = self.scoreState.totalScore;
    self.$score.text(score);
    self.$score.removeClass('positive negative');
    if (score > 0) {
      self.$score.addClass('positive');
    } else if (score < 0) {
      self.$score.addClass('negative');
    }
  };

  // Handle score threshold actions
  self.handleScoreThreshold = function(action) {
    if (action === 'stopRobot') {
