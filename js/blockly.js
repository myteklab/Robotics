var blockly = new function() {
  ...
  self.theme = Blockly.Theme.defineTheme('customTheme', {
    ...
    'startHats': true,
    'componentStyles': {
      'workspaceBackgroundColour': '#1e1e38',
      'toolboxBackgroundColour': '#16213e',
      'toolboxForegroundColour': '#e0e0e0',
      'flyoutBackgroundColour': '#252540',
      'flyoutForegroundColour': '#e0e0e0',
      'flyoutOpacity': 0.95,
      'scrollbarColour': '#3a3a5a',
      'scrollbarOpacity': 0.6,
      'insertionMarkerColour': '#7c8fff',
  ...
  this.init = function() {
    ...
    script.addEventListener('load', function() {
      // Register extension for keyboard_key_state block custom key input visibility
      Blockly.Extensions.register('keyboard_key_extension', function() {
        var block = this;
        var keyField = block.getField('key');
        var customKeyField = block.getField('customKey');

        function updateCustomKeyVisibility() {
          var selectedKey = keyField.getValue();
          var isCustom = (selectedKey === 'CUSTOM');

          if (customKeyField) {
            var fieldElement = customKeyField.getSvgRoot();
            if (fieldElement) {
              fieldElement.style.display = isCustom ? '' : 'none';
            }
          }
        }

        setTimeout(updateCustomKeyVisibility, 0);

        var originalValidator = keyField.getValidator();
        keyField.setValidator(function(newValue) {
          setTimeout(updateCustomKeyVisibility, 0);
          return originalValidator ? originalValidator.call(this, newValue) : newValue;
        });
      });
  ...
  this.loadCustomBlocks = function() {
    return fetch('customBlocks.json?v=2')
      .then(response => response.text())
      .then(function(response) {
  ...
  this.saveLocalStorage = function() {
    ...
    if (self.workspace && self.unsaved) {
      ...
      // Platform adapter hooks into window._roboticsBlocklySaveHook if defined
      if (window._roboticsBlocklySaveHook) {
        window._roboticsBlocklySaveHook();
      }
    }
  };
  ...
  // Load from local storage (standalone: empty, platform adapter calls loadXmlText with real data)
  this.loadLocalStorage = function() {
    // In standalone mode, load empty (default workspace already loaded).
    // Platform adapter will call blockly.loadXmlText() with saved data.
    self.loadXmlText('');
  };
