var pybricks_generator = new function() {
  ...
  this.load = function() {
    ...
    Blockly.Python['ledOff'] = self.ledOff;
    Blockly.Python['ledSetColor'] = self.ledSetColor;
    Blockly.Python['ledSetRange'] = self.ledSetRange;
    Blockly.Python['displayText'] = self.displayText;
    Blockly.Python['displayClear'] = self.displayClear;
    Blockly.Python['displaySetFont'] = self.displaySetFont;
    Blockly.Python['displaySetColor'] = self.displaySetColor;
    Blockly.Python['displayScrollText'] = self.displayScrollText;
    Blockly.Python['displayTypewriter'] = self.displayTypewriter;
    Blockly.Python['displayBlink'] = self.displayBlink;
    Blockly.Python['displayStopAnimation'] = self.displayStopAnimation;
    ...
    Blockly.Python['keyboard_key_state'] = self.keyboard_key_state;
  ...
  this.genCode = function() {
    ...
    while (sensor = robot.getComponentByPort('in' + i)) {
      if (sensor.type == 'ColorSensor') {
        ...
        sensorsCode += 'camera_in' + i + ' = CameraSensor(Port.S' + i + ')\n';
        self.autoPorts[sensor.type] = i;
      } else if (sensor.type == 'Display') {
        sensorsCode += 'display_in' + i + ' = Display(\'in' + i + '\')\n';
        self.autoPorts[sensor.type] = i;
      }
      i++;
    }
    ...
    while (motor = robot.getComponentByPort('out' + PORT_LETTERS[i])) {
      if (motor.type == 'MagnetActuator') {
        ...
        motorsCode += 'motor' + PORT_LETTERS[i] + ' = Motor(Port.' + PORT_LETTERS[i] + ') # Claw/Gripper\n';
        ...
        motorsCode += 'led' + PORT_LETTERS[i] + ' = LED(Port.' + PORT_LETTERS[i] + ')\n';
        self.autoPorts['LED'] = PORT_LETTERS[i];
      }
      i++;
    }
  ...
  this.cameraActivate = function(block) {
    ...
    var code = 'camera_in' + dropdown_port + '.activate()\n';
    return code;
  };

  this.cameraDeactivate = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'CameraSensor');

    var code = 'camera_in' + dropdown_port + '.deactivate()\n';
    return code;
  };

  this.ledOn = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'LED');

    var code = 'led' + dropdown_port + '.on()\n';
    return code;
  };

  this.ledOff = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'LED');

    var code = 'led' + dropdown_port + '.off()\n';
    return code;
  };

  this.ledSetColor = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'LED');
    ...
    var value_green = Blockly.Python.valueToCode(block, 'green', Blockly.Python.ORDER_ATOMIC);
    var value_blue = Blockly.Python.valueToCode(block, 'blue', Blockly.Python.ORDER_ATOMIC);
    var code = 'led' + dropdown_port + '.setColor(' + value_red + ', ' + value_green + ', ' + value_blue + ')\n';
    return code;
  };

  this.ledSetRange = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'LED');

    var value_range = Blockly.Python.valueToCode(block, 'range', Blockly.Python.ORDER_ATOMIC);
    var code = 'led' + dropdown_port + '.setRange(' + value_range + ')\n';
    return code;
  };

  // Display text on a specific line
  this.displayText = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_line = block.getFieldValue('line');
    var value_text = Blockly.Python.valueToCode(block, 'text', Blockly.Python.ORDER_ATOMIC);

    var code = 'display_in' + dropdown_port + '.text(' + dropdown_line + ', ' + value_text + ')\n';
    return code;
  };

  // Clear display (all lines or specific line)
  this.displayClear = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_line = block.getFieldValue('line');

    var code;
    if (dropdown_line === 'ALL') {
      code = 'display_in' + dropdown_port + '.clear()\n';
    } else {
      code = 'display_in' + dropdown_port + '.clear(' + dropdown_line + ')\n';
    }
    return code;
  };

  // Set display font size
  this.displaySetFont = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_size = block.getFieldValue('size');

    var code = 'display_in' + dropdown_port + '.set_font(\'' + dropdown_size + '\')\n';
    return code;
  };

  // Set display text color
  this.displaySetColor = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    ...
    var value_green = Blockly.Python.valueToCode(block, 'green', Blockly.Python.ORDER_ATOMIC);
    var value_blue = Blockly.Python.valueToCode(block, 'blue', Blockly.Python.ORDER_ATOMIC);
    var code = 'display_in' + dropdown_port + '.set_color(' + value_red + ', ' + value_green + ', ' + value_blue + ')\n';
    return code;
  };

  // Scrolling marquee text effect
  this.displayScrollText = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_line = block.getFieldValue('line');
    var value_text = Blockly.Python.valueToCode(block, 'text', Blockly.Python.ORDER_ATOMIC);
    ...
    var code = 'display_in' + dropdown_port + '.scroll_text(' + dropdown_line + ', ' + value_text + ', ' + value_speed + ')\n';
    return code;
  };

  // Typewriter effect
  this.displayTypewriter = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_line = block.getFieldValue('line');
    var value_text = Blockly.Python.valueToCode(block, 'text', Blockly.Python.ORDER_ATOMIC);
    ...
    var code = 'display_in' + dropdown_port + '.typewriter(' + dropdown_line + ', ' + value_text + ', ' + value_speed + ')\n';
    return code;
  };

  // Blinking text effect
  this.displayBlink = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_line = block.getFieldValue('line');
    var value_text = Blockly.Python.valueToCode(block, 'text', Blockly.Python.ORDER_ATOMIC);
    var value_onTime = Blockly.Python.valueToCode(block, 'onTime', Blockly.Python.ORDER_ATOMIC);
    var value_offTime = Blockly.Python.valueToCode(block, 'offTime', Blockly.Python.ORDER_ATOMIC);

    var code = 'display_in' + dropdown_port + '.blink(' + dropdown_line + ', ' + value_text + ', ' + value_onTime + ', ' + value_offTime + ')\n';
    return code;
  };

  // Stop animation
  this.displayStopAnimation = function(block) {
    var dropdown_port = block.getFieldValue('port');
    dropdown_port = self.getPort(dropdown_port, 'Display');
    var dropdown_line = block.getFieldValue('line');

    var code = 'display_in' + dropdown_port + '.stop_animation(' + dropdown_line + ')\n';
    return code;
  };
  ...
  this.keyboard_key_state = function(block) {
    let key = block.getFieldValue('key');
    let state = block.getFieldValue('state');

    // Handle custom key input - use customKey field value when CUSTOM is selected
    if (key === 'CUSTOM') {
      let customKey = block.getFieldValue('customKey');
      key = customKey || 'a'; // Use custom key or fallback to 'a'
    }

    // Escape the key for Python string
    let escapedKey = key.replace(/\\/g, '\\\\').replace(/'/g, "\\'");

    if (state == 'PRESSED') {
      var code = 'keyboard.is_pressed(\'' + escapedKey + '\')';
      return [code, Blockly.Python.ORDER_ATOMIC];
    } else {
      var code = 'not keyboard.is_pressed(\'' + escapedKey + '\')';
      return [code, Blockly.Python.ORDER_RELATIONAL];
    }
  };
